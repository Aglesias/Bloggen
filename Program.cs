                        ...
                        if (inlägg.Count > 0)
                        {
                            if (inlägg.Count > 0)
                            {
                                ...
                                for (int i = 0; i < max; i++)
                                {
                                    ...
                                    {
                                        int cmpVal = inlägg[j][0].CompareTo(inlägg[j + 1][0]); // Skapar ett värde som antingen är -1, 0, eller 1 genom att jämnföra ord i ordLista med varandra
                                        ...
                                        {
                                            string temp = inlägg[j][0];
                                            inlägg[j][0] = inlägg[j + 1][0];
                                            inlägg[j + 1][0] = temp;

                                            string temp1 = inlägg[j][1];
                                            inlägg[j][1] = inlägg[j + 1][1];
                                            inlägg[j + 1][1] = temp1;

                                            string temp2 = inlägg[j][2];
                                            inlägg[j][2] = inlägg[j + 1][2];
                                            inlägg[j + 1][2] = temp2;
                                        }
                            ...
                            if (sorteradeInlägg)
                            {
                                ...
                                while (first <= last)
                                {
                                    ...
                                    if (cmpVal > 0) // OM bokstaven som eftersöks är efter första bokstaven i "mellan" i alfabetet
                                    {
                                        first = (mellan + 1); // ändra värdet på first till mellan + 1
                                    }
                                    else if (cmpVal < 0)//  OM bokstaven som eftersöks är före första bokstaven i "mellan" i alfabetet
                                    {
